...
每日训练数据分析平台。核心是单个 Python 脚本,把两个数据源(力量训练 + 有氧/生理指标)融合成结构化 JSON 与 HTML 报告,按日期归档,推送到 GitHub 后由 GitHub Pages 部署展示。
...
1. **`fetch_xunji_data(date)`** — 通过 `subprocess` 调用外部 skill 脚本 `.Codex/skills/xunji-api/scripts/fetch_trains.py`,取力量训练数据,只取返回 JSON 的 `res` 字段。
...
- **xunji skill 自带按日缓存**:`fetch_trains.py` 把结果缓存在 `.Codex/skills/xunji-api/cache/`(按 `datestr`),同一天重复生成会命中缓存、不重新请求接口。若在训记 App 里改了数据但报告没更新,先清该目录再重跑。
- **部署链路**:`git push` 到 GitHub → GitHub Pages 自动构建(无需额外配置),报告经 `public/daily/{date}.html` 直接访问。
