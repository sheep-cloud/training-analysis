...
```bash
python .claude/skills/xunji-api/scripts/fetch_trains.py read --date 2026-04-02
python .claude/skills/xunji-api/scripts/fetch_trains.py read --date 2026-04-02 --full
```
...
2. 先执行 dry-run：
   ```bash
   python .claude/skills/xunji-api/scripts/fetch_trains.py upsert --file trains.json --dry-run
   ```
3. 向用户展示变更摘要（新增/更新/删除的训练、动作、组变化）。
4. 等待用户明确确认后再执行真实写回：
   ```bash
   python .claude/skills/xunji-api/scripts/fetch_trains.py upsert --file trains.json
   ```
...
```bash
python .claude/skills/xunji-api/scripts/fetch_trains.py movements
```
